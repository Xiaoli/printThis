...
(function($) {
  ...
  $.fn.printThis = function(options) {
    ...
    // add dynamic iframe to DOM
      ...
      last_iframe = $("<iframe id='" + strFrameName + "' src='about:blank'/>");
    ...
    setTimeout(function() {
      ...
      if (opt.loadCSS) {
        $doc.find("head").append("<link type='text/css' rel='stylesheet' href='" + opt.loadCSS + "'  media='print'>");
      }
      ...
      //$doc.close();
      // print
      ($.browser.opera ? tab : last_iframe[0].contentWindow).focus();
      setTimeout(function() {
        ($.browser.opera ? tab : last_iframe[0].contentWindow).print();
        if (tab) {
          tab.close();
        }
      }, 1000);
